...
uint16_t interval = 300; // Intervalo de impresión en ms
enum
{
	MODO_NORMAL = 0,
	MODO_AVG = 1,
...
// protocolo moved to comm_protocol module
...
void setup()
{
	...
	comm_init();	   // Inicializa handlers del protocolo (Wire.onRequest/Receive internamente)
...
void update_lcd_display()
{
	if (modo_display == MODO_AVG)
	{
		...
		lcd.print(" LUX = ");
		lcd.print(calculate_percentage(get_lux_avg()));
		lcd.print("%      ");
	}
