...
Este repositorio reúne el trabajo práctico, laboratorios experimentales y el proyecto final desarrollados para la cátedra **Sistemas Embebidos (2025)** del *Departamento de Ciencias e Ingeniería de la Computación (DCIC)* en la **Universidad Nacional del Sur (UNS)**.
...
| # | Módulo / Práctica | Plataforma & Hardware | Conceptos Clave & Tecnologías | Enlace |
|:---:|:---|:---|:---|:---:|
| **01** | **GPIO, Temporización & UART** | ATmega328P (Arduino Uno) | Entradas/salidas digitales, eliminación de rebotes (debouncing), retardos no bloqueantes, detección de flancos, comunicación serie asincrónica. | [📂 Explorar](01_GPIO_Temporizadores) |
| **02** | **Drivers Modulares & Colas de Eventos** | ATmega328P + LCD Keypad Shield | Capa de abstracción de hardware (HAL), secciones críticas atómicas, colas circulares de funciones (*Function Queue*), máquinas de estados para teclado analógico y cronómetro. | [📂 Explorar](02_Drivers_Mecanismos_Sync) |
| **03** | **Driver ADC por Interrupciones & GUI Host** | ATmega328P + Sensor LDR + Processing | Muestreo analógico no bloqueante por interrupción de ADC, búfer circular para estadísticas en tiempo real (mín/máx/promedio), protocolo UART bidireccional y dashboard en Processing. | [📂 Explorar](03_ADC_SensorLuminosidad) |
| **04** | **Comunicación I2C Distribuida (Master-Slave)** | Raspberry Pi 3B+ (Master) + Arduino (Slave) | Protocolo binario de longitud variable con enmarcado por delimitadores (`0x7E`/`0x7F`) y *byte-stuffing* (secuencias de escape), driver I2C en Linux con `pigpio` y app de consola en C++. | [📂 Explorar](04_I2C_RaspberryPi_Arduino) |
| **05** | **Sistemas Operativos de Tiempo Real (RTOS)** | ATmega328P + FreeRTOS | Planificación concurrente basada en prioridades, sincronización de tareas con semáforos, gestión de estados (Running, Ready, Blocked), colas de mensajes y temporizadores por software (*Software Timers*). | [📂 Explorar](05_FreeRTOS_Multitarea) |
| **06** | **Hardware Reconfigurable FPGA & SoPC Nios II** | Altera Cyclone V (DE1-SoC) | Síntesis RTL en Verilog, multiplexores con IPs `LPM_MUX`, red combinacional de ordenamiento en paralelo de 4 valores de 8 bits (*Sorter4*), integración de procesador soft-core Nios II Gen2 y HAL/no-HAL. | [📂 Explorar](06_FPGA_Verilog_SoPC) |
| **07** | ⭐ **Proyecto Final: Módem FT8 Autónomo (MAP-FT8)** | Espressif ESP32 Dual-Core (240 MHz) | Procesamiento Digital de Señales (DSP), KissFFT, matrices Costas, decodificación LDPC + CRC14, modulación continua CP-FSK, FreeRTOS, servidor web embebido y WebSockets. | [🔗 Repositorio Oficial](https://github.com/ManuT18/SistemasEmbebidos_ProyectoFinal) \| [📂 Submódulo](07_ProyectoFinal_ModemFT8) |
...
- **Resolución de Conflictos de Hardware:** Solución a la limitación interna de recursos I2S0 en ESP32 desacoplando el ADC mediante muestreo OneShot temporizado con `esp_timer` y buffers circulares libres de bloqueo (*lock-free*).
...
- **Manuel Tauro** — Estudiante de Ingeniería / Ciencias de la Computación (UNS)
...
- **Tomás Aguirre** — Co-autor de prácticas y proyecto final
